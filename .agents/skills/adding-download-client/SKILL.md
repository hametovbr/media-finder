...
4. Add a statically packaged module with:
   ...
   - typed Pydantic configuration and translation keys;
...
5. Resolve secrets only from `env:NAME`. Never return or log credentials, magnet URIs, torrent bytes, download URLs, passkeys, or raw upstream exception content.
...
9. Run focused tests, shared client conformance tests, type/lint checks, documentation-policy checks, and strict OpenSpec validation.
...
- Manifest, typed config, translations, fixtures
