...
Define versioned Pydantic public types and protocols in a core SDK package. A metadata provider exposes its manifest, capabilities, config validation, search, fetch, normalization, attribution, standardized errors, retention planning, provider-owned policy calculation, and a typed export-warning hook. Search, fetch, and normalize are uniform essential operations, must be advertised, and are always exercised successfully by conformance; the Manual provider uses in-memory fixture identities for this test boundary without acquiring database access. The warning hook can supply only deeply immutable allowlisted validated header values, and core revalidates them at consumption, allowing provider-owned retention notices without provider-specific branching. Core applies a planned refresh through the same public fetch, normalize, and policy operations and applies a planned purge generically; no raw provider payload is added to a public response model. A download client exposes its manifest, config validation, live destinations, artifact submission, and exact-correlation lookup.

The TMDB fetch operation treats a TV hierarchy as one provider snapshot: it requests the series details and every advertised season detail, including Season 00, before normalization. Its authenticated transport accepts only the canonical official HTTPS `api.themoviedb.org/3` base, explicit TMDB endpoint shapes, and numeric identities; the MVP has no proxy-origin override. Poster and backdrop file paths are converted to typed TMDB image URLs only after path validation.

Core owns orchestration and persistence through service interfaces. Modules receive data and narrowly scoped HTTP/runtime services, never database sessions or template directories. Environment-reference, registration, and conformance types live in the public SDK. One immutable compile-time registry supplies installed module config schemas, retention-only instances, and live builders to runtime and UI composition. Manifests reference typed config fields and translation keys; core renders generic forms. Shared fixture-driven conformance always exercises the essential metadata-provider operations and exercises only declared optional artifact forms for clients, so a client that advertises magnets but not torrent bytes is never tested against the unsupported form.
...
The revision envelope carries optional provider-supplied maintenance timestamps and status. Core runs a provider-agnostic maintenance coordinator at startup and once per day. It enumerates every installed registered provider type owning persisted revisions, including providers without current active configuration, asks each provider to plan due work, and applies the returned action in a savepoint dedicated to that revision. For refresh, core uses the provider's existing public fetch, normalize, and policy operations and persists the raw payload only as local orchestration material; for purge, core clears the derived payload generically. A successful refresh suppresses another refresh for that same envelope but does not remove it from planning, so the original provider-computed expiry can still cause mandatory purge. Any unexpected plan, action, identity, domain-validation, or persistence failure rolls back only its revision savepoint; a separate savepoint records the safe failure before later revisions continue.
...
Use a Prowlarr adapter for torrent-only interactive search. Cache normalized search results and their sensitive resolution data in a bounded process-memory TTL cache. Send the browser a random opaque token mapped to a cache entry. Restart or expiry intentionally invalidates outstanding selections. Bound form bodies, upstream JSON bytes, result counts, and torrent bytes before parsing or use. Stream bounded external bodies and consume a selected result token before artifact retrieval so even an oversized artifact cannot be retried with the same token.

On selection, reload client destinations, validate the requested destination, create a pending Acquisition transactionally, resolve the magnet or torrent bytes in memory, and call the client. Persist only a sanitized release snapshot. A GUID is retained only when the adapter classifies a bounded opaque value as non-sensitive; URL/path/credential-like or uncertain GUIDs are omitted. A source page must come from a dedicated public-page field. Its userinfo, query, fragment, path parameters, and non-normalized path segments are removed; when path provenance is uncertain, persist only the safe origin or omit the URL. Known and suspected passkeys and rejected values are removed before both persistence and logging. HTTP logger filtering removes complete integration URLs before records reach application handlers. TMDB, Prowlarr, and every qBittorrent instance receive separate cookie jars; safe reverse-proxy subpaths apply to Prowlarr and download clients, while TMDB is restricted to its official origin. If construction or validation fails, every HTTP client created by that attempt is closed and removed immediately.
